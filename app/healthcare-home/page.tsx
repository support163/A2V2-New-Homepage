...
import { Activity, Sparkles, Bell, ChevronDown, Search, Users, FileText, BarChart2, Shield, UserCog, ClipboardList, Check, MessageSquare } from 'lucide-react'
...
function LongevityDashboard() {
  return (
    <div className="flex h-full">
      <DashboardSidebar
        navItems={[
          { label: 'Patient queue', icon: <Users size={12} />, active: false },
          { label: 'Active protocols', icon: <FileText size={12} />, active: true },
          { label: 'Lab tracking', icon: <BarChart2 size={12} />, active: false },
          { label: 'Adherence', icon: <Activity size={12} />, active: false },
          { label: 'Reports', icon: <Shield size={12} />, active: false },
        ]}
        subListLabel="Protocols"
        subListItems={['NAD+ IV Therapy', 'Peptide BPC-157', 'Glutathione Push']}
      />
      <div className="flex flex-col flex-1 overflow-hidden" style={{ padding: '14px' }}>
        <DashboardTopBar title="Scenario: NAD+ Week 3 Check-in" badge="Draft" />
        <ScenarioBlock
          title="Scenario 1: Week 3 check-in"
          rows={[
            { type: 'Say', content: "How are you feeling after this week's infusion?" },
            { type: 'If', content: <><Var>positive_response</Var> → Continue protocol</> },
            { type: 'If', content: <><Var>side_effects</Var> → Escalate to provider</> },
          ]}
        />
        <ScenarioBlock
          title="Scenario 2: Lab reminder"
          rows={[
            { type: 'Say', content: 'Your follow-up labs are due in 5 days.' },
            { type: 'Trigger', content: <><Var>labs_booked</Var> → Mark complete</> },
            { type: 'Action', content: 'Send reminder if no response in 48h' },
          ]}
        />
      </div>
    </div>
  ...
  return (
    <div className="flex h-full">
      <DashboardSidebar
        navItems={[
          { label: 'Patient queue', icon: <Users size={12} />, active: false },
          { label: 'Hormone protocols', icon: <FileText size={12} />, active: true },
          { label: 'Lab schedules', icon: <BarChart2 size={12} />, active: false },
          { label: 'Compliance', icon: <Shield size={12} />, active: false },
        ]}
        subListLabel="Protocols"
        subListItems={['Testosterone optimization', 'Estrogen balance', 'Thyroid']}
      />
      <div className="flex flex-col flex-1 overflow-hidden" style={{ padding: '14px' }}>
        <DashboardTopBar title="Scenario: HRT Month 2 Review" badge="Draft" />
        <ScenarioBlock
          title="Scenario 1: Month 2 check-in"
          rows={[
            { type: 'Say', content: "It's been 60 days — how are your energy levels?" },
            { type: 'If', content: <><Var>symptom_improvement</Var> → Continue dose</> },
            { type: 'If', content: <><Var>adverse_effect</Var> → Flag for review</> },
          ]}
        />
        <ScenarioBlock
          title="Scenario 2: Lab follow-up"
          rows={[
            { type: 'Say', content: 'Your hormone panel results are ready to review.' },
            { type: 'Trigger', content: <><Var>appointment_booked</Var> → Confirm</> },
            { type: 'Action', content: 'Notify provider if labs are out of range' },
          ]}
        />
      </div>
    </div>
    ...
    <div className="flex h-full">
      <DashboardSidebar
        navItems={[
          ...
          { label: 'Lab work', icon: <BarChart2 size={12} />, active: false },
        ]}
        subListLabel="Protocols"
        subListItems={['Gut healing', 'Adrenal support', 'Detox']}
      />
      <div className="flex flex-col flex-1 overflow-hidden" style={{ padding: '14px' }}>
        <DashboardTopBar title="Scenario: Multi-protocol coordination" badge="Active" />
        <ScenarioBlock
          title="Scenario 1: Gut healing check-in"
          rows={[
            { type: 'Say', content: 'How is your digestion responding to the protocol?' },
            { type: 'If', content: <><Var>bloating_reported</Var> → Adjust probiotics</> },
            { type: 'Trigger', content: <><Var>week_4_complete</Var> → Begin phase 2</> },
          ]}
        />
        <ScenarioBlock
          title="Scenario 2: Adrenal support"
          rows={[
            { type: 'Say', content: 'Rate your energy levels today from 1 to 10.' },
            { type: 'If', content: <><Var>score_below_5</Var> → Flag provider</> },
            { type: 'Action', content: 'Log response and update care plan' },
          ]}
        />
      </div>
    </div>
    ...
    <div className="flex h-full">
      <DashboardSidebar
        navItems={[
          ...
          { label: 'Wearable data', icon: <Activity size={12} />, active: false },
          { label: 'Biomarkers', icon: <BarChart2 size={12} />, active: false },
        ]}
        subListLabel="Programs"
        subListItems={['Executive panel', 'VO2 max training', 'Sleep optimization']}
      />
      <div className="flex flex-col flex-1 overflow-hidden" style={{ padding: '14px' }}>
        <DashboardTopBar title="Scenario: Quarterly biomarker review" badge="Scheduled" />
        <ScenarioBlock
          title="Scenario 1: Q1 biomarker review"
          rows={[
            { type: 'Say', content: 'Your Q1 biomarker report is ready for review.' },
            { type: 'If', content: <><Var>appointment_confirmed</Var> → Send prep guide</> },
            { type: 'If', content: <><Var>no_response_48h</Var> → Send reminder</> },
          ]}
        />
        <ScenarioBlock
          title="Scenario 2: VO2 max check-in"
          rows={[
            { type: 'Say', content: "How did this week's training sessions feel?" },
            { type: 'Trigger', content: <><Var>hrv_drop_detected</Var> → Reduce intensity</> },
            { type: 'Action', content: 'Sync wearable data and update plan' },
          ]}
        />
      </div>
    </div>
...
const showcaseTabs = [
  { title: 'Longevity clinics', description: 'NAD+, peptides, biomarker tracking, and supplement protocols built in.' },
  { title: 'HRT practices', description: 'Hormone optimization sequences with cycle-aware reminders and lab tracking.' },
  { title: 'Functional medicine', description: 'Multi-protocol support for complex root-cause treatment plans.' },
  { title: 'Health optimization', description: 'Executive health programs with wearable integration and biomarker trends.' },
]

const showcaseSummary = [
  { Icon: Activity, title: 'Engagement', desc: 'Protocol-aware' },
  { Icon: Sparkles, title: 'Intelligence', desc: 'Native protocols' },
  { Icon: Bell, title: 'Alerts', desc: 'Predictive warnings' },
]
